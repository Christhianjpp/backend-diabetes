...
export interface IUserReport {
  ...
  commentId?: string;
  noteId?: string;
  reason: 'spam' | 'harassment' | 'inappropriate_content' | 'hate_speech' | 'fake_information' | 'other';
  description?: string;
...
UserReportSchema.index(
  { reporterId: 1, reportedUserId: 1, commentId: 1 },
  { unique: true, partialFilterExpression: { commentId: { $exists: true } } }
);
UserReportSchema.index(
  { reporterId: 1, reportedUserId: 1, noteId: 1 },
  { unique: true, partialFilterExpression: { noteId: { $exists: true } } }
);
